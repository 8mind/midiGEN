...
**NOTE: the files `model.py`, `configurator.py` are taken VERBATIM from [`karpathy/nanoGPT`](https://github.com/karpathy/nanoGPT/tree/master). `train.py` is also taken verbatim from `karpathy/nanoGPT` but for two lines: line 263 and 276. `sample.py` is also from `karpathy/nanoGPT` but is ~50% changed.**
